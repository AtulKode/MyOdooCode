...
print("uid \n\n",uid)
work_book = xlrd.open_workbook("/home/jatin/Downloads/purchase.xls")
...
for sheet in work_book._sheet_list:
    ...
    for sheet_data in sheet_values[1:]:
        order_ref = sheet_data[0].encode('utf-8')
        # date = sheet_data[1]
        # print ("order_ref==>>", order_ref,date)
        order_id = sock.execute_kw(dbname, uid, pwd, 'purchase.order', 'search', [[('name', '=', order_ref)]])
        print ("order_id===>>", order_id)
        if order_id:
            order_date = sheet_data[1]
            if isinstance(order_date, (int, float)):
                order_date = excel_serial_to_date(order_date)
            else:
                order_date = order_date
            print ("order_date====>>", order_date)
            sock.execute_kw(dbname, uid, pwd, 'purchase.order', 'write', [order_id, {
                'date_approve': order_date
            }])
            completed_ref.append(order_ref)
            print ("Order Date is changed : ", order_ref)
        else: 
            order_ref_list.append(order_ref)
            print ("Order Not Found : ", order_ref)

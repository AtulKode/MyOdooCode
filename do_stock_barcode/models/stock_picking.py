...
class StockPicking(models.Model):
    ...
    def on_barcode_scanned(self, barcode):
        print("\n\n On Barcode Scanned Method 1111====", self, barcode)
        product = self.env['product.product'].search(
            ['|', ('barcode', '=', barcode), ('default_code', '=', barcode)], limit=1)
        if product:
            moveLines = self.move_ids_without_package.mapped('move_line_ids').filtered(
                lambda r: r.product_id == product)
            for line in moveLines:
                line.move_id.quantity = line.move_id.quantity + 1
                self.env.cr.commit()
                return {'warning': {'title': _('Successfully Added'),
                                    'message': _(' %s - %s/%s.') % (product.name,  line.move_id.quantity,  line.move_id.product_uom_qty)
                                    },
                        }

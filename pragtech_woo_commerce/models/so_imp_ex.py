    ...
    def export_selected_so(self, instance_id):
        ...
        for rec in records:
            ...
            for lines in rec.order_line:
                ...
                if lines.product_id.woo_id or lines.product_id.product_tmpl_id.woo_id:
                    if not lines.w_id:
                        order_lines.append({
                            "product_id": lines.product_id.woo_id or lines.product_id.product_tmpl_id.woo_id,
                            "quantity": lines.product_uom_qty,
                            "sku": lines.product_id.default_code if lines.product_id.default_code else '',
                            "price": lines.price_unit,
                            "total": str(lines.price_unit),
                            "total_tax": str(lines.price_tax),
                            "taxes": tax_lines,
                        })
                                        ...
                                        if not product:
                                            product = self.env['product.template'].sudo().create({
                                                ...
                                                'description': product.description_sale if product and product.description_sale else i.get(
                                                    'name'),
                                                'description_sale': product.description_sale if product and product.description_sale else i.get(
                                                    'name'),
